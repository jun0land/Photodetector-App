...
DEFAULTS = {
    ...
    "use_abs": True,
    # 엑셀 내보내기에서 전류를 |I| 로 낼지. Origin 로그축에 바로 올리려면 양수여야 하므로
    # **기본 켜짐**. Raw 시트만은 예외로 늘 부호를 유지한다 — 원본 보존이 그 시트의 역할.
    "export_abs": True,
    ...
    "postproc": {
        ...
        # 부호 전환점을 가로질러 평활하면 로그축 골짜기가 메워진다(실측 최대 3.4배).
        # 부호가 같은 구간끼리만 평활한다. 끄면 예전처럼 통째로 평활.
        "split_sign": True,
    },
